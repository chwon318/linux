...
OPENAI_CHAT_ERROR_MESSAGE = (
    "AI 연결에 실패했습니다. services.py의 OPENAI_API_KEY 값과 인터넷 연결 상태를 확인해주세요."
)
OPENAI_DIARY_ERROR_TITLE = "AI 연결 확인 필요"
OPENAI_DIARY_ERROR_BODY = (
    "AI 일기 생성에 실패했습니다.\n\n"
    "services.py에 작성된 OPENAI_API_KEY 값이 올바른지 확인하고, "
    "OpenAI 계정의 사용 가능 여부와 인터넷 연결 상태를 확인해주세요."
)
...
def log_openai_error(action, error):
    if isinstance(error, urllib.error.HTTPError):
        try:
            detail = error.read().decode("utf-8")
        except Exception:
            detail = str(error)
        logger.warning("OpenAI %s failed: HTTP %s %s", action, error.code, detail)
        return
    logger.warning("OpenAI %s failed: %s", action, error)
...
def generate_follow_up(conversation):
    ...
    except (RuntimeError, urllib.error.URLError, urllib.error.HTTPError, KeyError, TimeoutError) as error:
        log_openai_error("chat", error)
        return OPENAI_CHAT_ERROR_MESSAGE
...
def generate_diary(conversation):
    try:
        ...
        log_openai_error("diary", error)
        return OPENAI_DIARY_ERROR_TITLE, OPENAI_DIARY_ERROR_BODY
